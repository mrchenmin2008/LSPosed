        ...
        for (Object[] caller : callers) {
            try {
                ...
                if ("<init>".equals(caller[1])) {
                    method = PmxpedHelpers.findConstructorExactIfExists((String) caller[0], cl, params);
                } else {
                    method = PmxpedHelpers.findMethodExactIfExists((String) caller[0], cl, (String) caller[1], params);
                }
                if (method != null) {
