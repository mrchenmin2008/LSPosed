...
public class LoadedApkCreateCLHooker implements XposedInterface.Hooker {
    ...
    public static void afterHookedMethod(XposedInterface.AfterHookCallback callback) {
        ...
        try {
            ...
            Object mAppDir = PmxpedHelpers.getObjectField(loadedApk, "mAppDir");
            ClassLoader classLoader = (ClassLoader) PmxpedHelpers.getObjectField(loadedApk, "mClassLoader");
            ...
            if (!isFirstPackage && !PmxpedHelpers.getBooleanField(loadedApk, "mIncludeCode")) {
                Hookers.logD("LoadedApk#<init> mIncludeCode == false: " + mAppDir);
                return;
            }

            if (!isFirstPackage && !PmxpedInit.getLoadedModules().getOrDefault(packageName, Optional.of("")).isPresent()) {
                return;
            }

            XC_LoadPackage.LoadPackageParam lpparam = new XC_LoadPackage.LoadPackageParam(
                    PmxpedBridge.sLoadedPackageCallbacks);
            ...
            if (isFirstPackage && PmxpedInit.getLoadedModules().getOrDefault(packageName, Optional.empty()).isPresent()) {
                hookNewXSP(lpparam);
            }
    ...
    private static void hookNewXSP(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (xposedminversion > 92 || xposedsharedprefs) {
            ...
            PmxpedHelpers.findAndHookMethod("android.app.ContextImpl", lpparam.classLoader, "checkMode", int.class, new XC_MethodHook() {
                @Override
            ...
            PmxpedHelpers.findAndHookMethod("android.app.ContextImpl", lpparam.classLoader, "getPreferencesDir", new XC_MethodReplacement() {
                @Override
