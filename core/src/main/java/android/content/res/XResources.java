...
import static de.robv.android.pmxped.PmxpedHelpers.decrementMethodDepth;
import static de.robv.android.pmxped.PmxpedHelpers.findAndHookMethod;
import static de.robv.android.pmxped.PmxpedHelpers.getBooleanField;
import static de.robv.android.pmxped.PmxpedHelpers.getLongField;
import static de.robv.android.pmxped.PmxpedHelpers.getObjectField;
import static de.robv.android.pmxped.PmxpedHelpers.incrementMethodDepth;
...
import de.robv.android.pmxped.IPmxpedHookZygoteInit;
import de.robv.android.pmxped.XC_MethodHook;
import de.robv.android.pmxped.XC_MethodHook.MethodHookParam;
import de.robv.android.pmxped.PmxpedBridge;
import de.robv.android.pmxped.PmxpedBridge.CopyOnWriteSortedSet;
import de.robv.android.pmxped.PmxpedInit;
import de.robv.android.pmxped.callbacks.XC_LayoutInflated;
import de.robv.android.pmxped.callbacks.XC_LayoutInflated.LayoutInflatedParam;
...
//			Log.w(XposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link IPmxpedHookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { PmxpedBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						PmxpedBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					PmxpedBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				PmxpedBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				PmxpedBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			PmxpedBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
